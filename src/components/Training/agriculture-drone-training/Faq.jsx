...
const Faq = () => {
  ...
  return (
    <section>
      ...
      <div>
        <div className="relative flex flex-col justify-center gap-20 py-0 pb-8 text-center md:py-8 section-wrapper md:px-8 md:flex-row">
          <div className="grid min-w-full grid-cols-1 gap-4 lg:grid-cols-2 md:gap-x-12 md:gap-y-6">
            {faqs.map((faq, index) => (
              <div
                key={index}
                className={`overflow-hidden border border-[#A8A53D] rounded-xl font-body ${
                  openIndex !== index ? "max-h-fit" : ""
                }`}
              >
                <button
                  onClick={() => toggleFAQ(index)}
                  className="flex items-center justify-between w-full gap-2 p-4 py-5 font-semibold text-left transition duration-300 bg-white hover:bg-gray-100 text-[#48655D]"
                >
                  {faq.question}
                  <Image
